<?php
/*
Plugin Name: HTTP API: GET Request with transient
...
function http_get_response() {
	...
	//the response will receive all the different parts of the request
	if ( false === ( $response = get_transient( 'jsonplaceholder' ) ) ) {
		$response = http_get_request( $url );
		set_transient('jsonplaceholder', $response, HOUR_IN_SECONDS); 
	}

	// response data which is broken down into various parts from the request
